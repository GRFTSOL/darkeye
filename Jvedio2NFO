...
if DOWNLOAD_IMAGES:
    try:
        ...
        raise RuntimeError("DOWNLOAD_IMAGES=True 但未安装 requests。请在解释器里 pip install requests 并重启脚本。")
...
def pretty_xml(elem):
    ...
    # 1. 替换自带的 XML 声明为标准格式
    xml_str = re.sub(r'^<\?xml.*?\?>', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>', xml_str)
