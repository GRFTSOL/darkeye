...
def setup_mainwindow_actions(window, registry):
    ...
    from ui.dialogs.open import (
        open_add_masturbation_dialog,
        open_add_quick_work_dialog,
        open_add_make_love_dialog,
        open_add_sexual_arousal_dialog,
    )
    ...
    add_masturbation.triggered.connect(open_add_masturbation_dialog)
    registry.actions_map["add_masturbation_record"] = add_masturbation

    add_quick_work = QAction("快速添加番号", window)
    add_quick_work.setShortcut(QKeySequence(registry.get_shortcut("add_quick_work")))
    add_quick_work.triggered.connect(open_add_quick_work_dialog)
    registry.actions_map["add_quick_work"] = add_quick_work

    add_makelove = QAction("添加做爱记录", window)
    add_makelove.setShortcut(QKeySequence(registry.get_shortcut("add_makelove_record")))
    add_makelove.triggered.connect(open_add_make_love_dialog)
    ...
    add_sexual_rousal.triggered.connect(open_add_sexual_arousal_dialog)
    registry.actions_map["add_sexual_rousal_record"] = add_sexual_rousal

    open_help_action = QAction("打开帮助", window)
    open_help_action.setShortcut(QKeySequence(registry.get_shortcut("open_help")))
    ...
    focus_search.setShortcut(QKeySequence(registry.get_shortcut("search")))
    ...
    all_capture.setShortcut(QKeySequence(registry.get_shortcut("allcapture")))
    ...
    capture.setShortcut(QKeySequence(registry.get_shortcut("capture")))
