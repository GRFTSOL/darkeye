--v2.0版本,现在要升级。
--添加新的表series,一个作品最多只属于一个系列，但是一个系列可以有多个作品。
--作品添加runtime时长字段，单位是分钟
--女优添加notes字段
--男优添加notes字段
--修改work story的字段为notes,添加label_id字段,maker_id字段，series_id字段,fanart字段
--label添加aliases字段
...
CREATE TABLE IF NOT EXISTS label(--厂牌，レーベル，一部作品只有一部厂牌，一部厂牌可以有多个作品。一部作品可以没有厂牌
	label_id INTEGER PRIMARY KEY AUTOINCREMENT,--不重复主键
	cn_name TEXT,										--中文名
	jp_name TEXT,										--日文名
	aliases TEXT,										--别名,中间用,分开
	detail TEXT                                       --其他信息
);

CREATE TABLE IF NOT EXISTS maker(--制作商，一部片子只有一个制作商，一部制作商可以有多个作品。一部作品可以没有制作商
...
CREATE TABLE IF NOT EXISTS series(--系列表，一个作品最多只属于一个系列，但是一个系列可以有多个作品。一部作品可以没系列
	series_id INTEGER PRIMARY KEY AUTOINCREMENT,--不重复主键
	cn_name TEXT,										--中文名
	jp_name TEXT,										--日文名
	aliases TEXT,										--别名,中间用,分开
	detail TEXT,                                       --其他信息
	related_series TEXT                                --相关的系列，中间用,分开，指向series_id
);
...
CREATE TABLE IF NOT EXISTS "work" (--公有作品表新的表
    ...
    director TEXT,                     --导演
    runtime INTEGER,                   --时长，单位是分钟
    notes TEXT,                        --这个主要是自己写的剧情
	...
	cn_story TEXT,                     --翻译剧情
	jp_story TEXT,                     --官方剧情
	maker_id INTEGER,                  --制作商id
	label_id INTEGER,                  --厂牌id
	series_id INTEGER,                  --系列id
	fanart TEXT,                        --json格式，存储其他的图片，主要是兼容nfo,[{"url":"htpps://xxx.jpg","file":"xxx.jpg"},{"url":"htpps://xxx.jpg","file":"xxx.jpg"}]
