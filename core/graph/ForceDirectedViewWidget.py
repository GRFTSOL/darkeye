...
class ForceDirectedViewWidget(QWidget):
    ...
    def signal_connect(self):
        ...
        if self._theme_manager is not None:
            self._theme_manager.themeChanged.connect(self._apply_background_from_tokens)
            self._apply_background_from_tokens()
    ...
    @Slot()
    def _apply_background_from_tokens(self) -> None:
        """根据当前主题令牌设置 OpenGL 视图背景色（color_bg_page 表示页面背景）。"""
        if self._theme_manager is None:
            return
        tokens = self._theme_manager.tokens()
        color_str = getattr(tokens, "color_bg_page", None) or getattr(tokens, "color_bg", "#ffffff")
        c = QColor(color_str)
        if c.isValid():
