...
def _radius_from_degree(degrees: dict, node_id, min_deg: float, max_deg: float) -> float:
...
将全局 diff 按当前 filter 进行裁剪/转换，得到会话内子图的增量 diff，然后发给 UI。
    ...
    def new_load(self):
        ...
        self.data_ready.emit({"cmd": "load_graph", "graph": self.sub_G, "modify": False})
    ...
    def _on_global_diff(self, global_diff_list: list):
        ...
        for u, v in new_sub_G.edges():
            if not self.sub_G.has_edge(u, v):
                diff_list.append({'op': 'add_edge', 'u': u, 'v': v, 'attr': new_sub_G.edges[u, v]})
