...
class ImageOverlayWidget(QWidget):
    ...
    def _update_window_flags(self, transparent_input: bool):
        ...
        if transparent_input:
            ...
            flags = Qt.WindowType.FramelessWindowHint | Qt.WindowType.WindowTransparentForInput
        else:
    ...
    def on_node_hovered_with_info(self, node_id: str, x: float, y: float,
                                radius: float, scale: float, dragging: bool):
        """
        处理悬停事件，显示或隐藏图片
    ...
    def _calculate_image_rect(self, x: float, y: float, radius: float, 
                          scale: float, node_id: str) -> QRectF:
        """
        计算图片在场景坐标系中的显示矩形
    ...
    def update_position_from_node_id(self, node_id: str, view_widget) -> bool:
        ...
        try:
            ...
            # 获取scale和radius
            scale = view_widget.getZoom() if hasattr(view_widget, 'getZoom') else self._last_scale
            radius = 7.0  # 默认半径
