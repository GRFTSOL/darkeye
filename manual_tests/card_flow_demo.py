...
class CardItem(QGraphicsObject):
    ...
    def paint(self, painter, option, widget):
        ...
        # 2. 绘制占位图片区域（上半部分）
        img_rect = QRectF(
            rect.left() + 10, rect.top() + 10, rect.width() - 20, rect.height() * 0.45
        )
        ...
        # 计算标题区域
        title_rect = QRectF(
            rect.left() + 10, img_rect.bottom() + 15, rect.width() - 20, 30
        )
        ...
        desc_rect = QRectF(
            ...
            rect.width() - 30,
            rect.height() - title_rect.bottom() - 20,
        )
        painter.drawText(
            desc_rect, Qt.AlignHCenter | Qt.AlignTop | Qt.TextWordWrap, self.desc_text
        )
