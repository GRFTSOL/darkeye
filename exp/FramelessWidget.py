...
SetWindowPos.argtypes = [
    wintypes.HWND, wintypes.HWND, c_int, c_int, c_int, c_int, wintypes.UINT
]
...
class TitleBar(QWidget):
    ...
    def _init_ui(self):
        ...
        self._close_btn = TitleBarButton(
            icon_path=f"{icon_path}/close.svg" if icon_path else "",
            button_type="close"
        )
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self._is_dragging and self._drag_start_pos and self._parent_window:
            ...
            if self._parent_window.isMaximized():
                # 计算还原后的位置
                self._parent_window._restore_from_maximized(event.globalPosition().toPoint())
                self._drag_start_pos = event.globalPosition().toPoint()
            else:
...
class FramelessWidget(QWidget):
    ...
    def _restore_from_maximized(self, cursor_pos: QPoint):
        ...
        cursor_x_ratio = (cursor_pos.x() - self.x()) / title_bar_width if title_bar_width > 0 else 0.5
...
if __name__ == "__main__":
    ...
    maximize_btn.clicked.connect(lambda: window.showNormal() if window.isMaximized() else window.showMaximized())
    ...
    # 状态信息
    status_label = QLabel("DWM 状态: " + ("已启用" if is_dwm_composition_enabled() else "未启用（使用回退方案）"))
