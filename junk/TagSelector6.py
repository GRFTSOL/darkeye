...
from ui.basic import WaterfallLayout,VLabel
...
class TagSelector6(QWidget):
    ...
    def __init__(self,enbale_mutex_check=True):
        ...
        self.selected_ids:dict[int,bool]={} #选择的ids
    ...
    def move_to_left(self, label:VerticalTagLabel2):
        ...
        if label.tag_id not in self.selected_ids:
            ...
            self.selected_ids[label.tag_id] = True
    ...
    def restore_to_right(self, label:VerticalTagLabel2):
        ...
        if label.tag_id in self.selected_ids:
            ...
            self.type_map[label.tag_type].addWidget(label)
            del self.selected_ids[label.tag_id]
            self.selection_changed.emit()
            #logging.debug(self.selected_ids)
    ...
    def load_with_ids(self, ids: list[int]):
        """根据 id 列表同步 UI"""
        new_ids = set(ids)
        current_ids = set(self.selected_ids.keys())
    ...
    def get_selected_ids(self) -> list[int]:
        """获得已选中的 tag id"""
        return list(self.selected_ids.keys())
...
#-----------------------------------------------------
    ...
    def clear_left_tags(self):
        """清除左侧所有标签，恢复到右侧"""
        for tag_id in list(self.selected_ids.keys()):  # 转成 list 防止迭代修改
