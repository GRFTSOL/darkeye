...
- 利用にあたっては、中華人民共和国の関連法令（《网络安全法》《数据安全法》《个人信息保护法》《著作权法》など）および適用される規定を遵守してください。
...
| **複数ソース・クローラー** | javlib、avdanyuwiki、javtxt、javdb、minnano-av など。取得可否と品質は対象サイトの公開方針とアクセスルールに依存 | ✅ |
...
- **データと通信**：既定では実行ファイル近くの `data/` に保存（DB・設定・カバー・ポートレートなど。設定で変更可）。ローカル資料を第三者へ自動送信することはありません。通信は主にクローラーとリソースの取得、任意の更新確認（Cloudflare R2）、翻訳（Google または設定した LLM API）などで発生します。サードパーティサービスは利用者が任意で有効化し、適法性の確認責任を負います。
