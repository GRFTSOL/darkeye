...
(function () {
  ...
  function parse_data_javlibrary() {
    if (window.location.href.includes("javlibrary.com")) {
      ...
      data.genre = Array.from(genreElements).map((el) =>
        el.textContent.trim()
      );

      const castElements = document.querySelectorAll("#video_cast .star a");
      data.actress = Array.from(castElements).map((el) =>
        el.textContent.trim()
      );
      ...
      if (previewThumbs) {
        ...
        data.fanart = Array.from(anchors)
          .map((a) => {
  ...
  if (sessionStorage.getItem("darkeye_auto_parse") === "true") {
    ...
    const isVlSearchById = window.location.href.startsWith(
      "https://www.javlibrary.com/cn/vl_searchbyid.php?keyword="
    );
    if (isVlSearchById) {
      ...
      setTimeout(() => {
        console.log("DarkEye: 搜索页接力（例如已通过验证），继续执行...");
        search_javlibrary();
      }, 800);
    } else {
      setTimeout(() => {
