...
class TagGraphicsItem(QGraphicsObject):
    ...
    def __init__(self, tag_id, text, tag_type, background_color, detail, tag_mutex, parent=None):
    ...
    def _calculate_geometry(self):
        ...
        if not self.text_content:
            ...
            height = total_height + width * self.corner_cut_ratio * 3 + \
                     width * self.hole_radius_ratio * 2 - \
                     fm_first.descent() * 0.3 - fmodify
        ...
        hole_path.addEllipse(QPointF(hole_center_x, hole_center_y), hole_radius, hole_radius)
    ...
    def paint(self, painter, option, widget=None):
        ...
        if self._flashing:
            ...
            bg_color = QColor(self.text_color) if isinstance(self.text_color, str) else self.text_color
            text_color = QColor(self.bg_color) if isinstance(self.bg_color, str) else self.bg_color
            stroke_color = QColor(self.border_color) if isinstance(self.border_color, str) else self.border_color
        elif self._hovered:
...
class TagWaterfallScene(QGraphicsScene):
    ...
    def layout_items(self, width, current_type=None, custom_items=None):
        ...
        # 居中偏移
        total_content_width = col_count * self.column_width + (col_count - 1) * self.spacing
...
class TagWaterfallView(QGraphicsView):
    def __init__(self, scene, parent=None):
        ...
        self.setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded) # 改为按需显示，或保持 AlwaysOn 但需确保 SceneRect 正确
    ...
    def showEvent(self, event):
        ...
        if self.scene() and self.viewport().width() > 0:
            if self.custom_items is not None:
                self.scene().layout_items(self.viewport().width(), custom_items=self.custom_items)
            else:
                self.scene().layout_items(self.viewport().width(), self.current_tag_type)
    ...
    def update_layout(self):
        ...
        if self.scene():
            ...
            if width < 50: width = self.width() # Fallback
...
class TagSelector5(QWidget):
    ...
    def __init__(self, enbale_mutex_check=True):
        super().__init__()
        self.setCursor(QCursor(QPixmap(Path(ICONS_PATH/"mouse_off.png")),hotX=32,hotY=32))
        self.setStyleSheet("""
    ...
    def load_tags(self):
        ...
        for tag_id, name, tag_type, color, detail, tag_mutex in tags:
            # 存储纯数据
            tag_data = {
                'id': tag_id, 'name': name, 'type': tag_type, 
                'color': color, 'detail': detail, 'mutex': tag_mutex
            }
    ...
    def move_to_left(self, tag_id):
        ...
        if tag_id in self.selected_ids: return
    ...
    def restore_to_right(self, tag_id, switch_tab=True):
        """从左侧移回右侧"""
        if tag_id not in self.selected_ids: return
    ...
    def check_mutex_with_selected(self, new_tag_data) -> str|None:
        """互斥检查"""
        if not new_tag_data['mutex']: return None
