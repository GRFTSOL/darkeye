...
"""
    ...
    def calc_radar_value(self, actress: dict, data: list[dict]):
        ...
        if (
            actress["bust"] is None
            or actress["cup"] is None
            or actress["height"] is None
            or actress["hip"] is None
            or actress["waist"] is None
            or actress["cup"] == ""
        ):
            return [0, 0, 0, 0, 0]
    ...
    def calc_chain(self, actress_id):
        ...
        start_nodes = [
            n for n in nodelist if n["id"] not in all_redirects
        ]  # 找到开始的节点，默认只有单链
