...
class ViewModel(QObject):
    ...
    def _clear_all_info(self):
        ...
        default_type_id = self.find_tag_type_id_by_value("默认分类") or next(
            iter(self.tag_type_map), None
        )
    ...
    def find_tag_type_id_by_value(self, target_value):
        """根据值查找第一个对应的键"""
        return next(
            (key for key, value in self.tag_type_map.items() if value == target_value),
            None,
        )
...
class TagManagement(LazyWidget):
    ...
    def _lazy_load(self):
        ...
        self.btn_redirect.setToolTip(
            "将选择器中第一个标签重定向到第二个标签，此时第一个标签将成为第二个标签的别名"
        )
    ...
    def bind_model(self):
        ...
        self.tag_type.currentTextChanged.connect(
            lambda tag_type: self.vm.set_tag_type_id(
                self.vm.find_tag_type_id_by_value(tag_type)
            )
        )  # 改变model
        ...
        bindings_map2 = {"tag_name": self.tag_name, "tag_detail": self.detail}
    ...
    def createlivetag(self):
        ...
        if not self.livetaglabel:
            self.livetaglabel = VShowTagLabel(
                self.vm.get_tag_id() or -1,
                self.vm.get_tag_name(),
                self.vm.get_tag_color(),
                self.vm.get_tag_detail(),
            )
    ...
    def on_tag_change(self):
        ...
        if len(ids) == 0:
            ...
            if not self.taglabel:
                self.taglabel = VerticalTagLabel(
                    ids[0], data["tag_name"], data["color"], data.get("detail", "")
                )
    ...
    def redirect_tag_121(self):
        ...
        box.setText(
            "请选择要将哪个标签合并到哪个（被合并的标签会被重定向到保留的标签）："
        )
        ...
        logging.info(
            f"标签重定向,1对1,{get_taginfo_by_id(from_id).get('tag_name')}->>{get_taginfo_by_id(to_id).get('tag_name')}"
        )
