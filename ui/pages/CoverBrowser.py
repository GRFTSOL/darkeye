...
class GradientOverlay(QWidget):
    ...
    def paintEvent(self, event):
        ...
        gradient = QLinearGradient(
            QPointF(window_width - 0.8 * self._scaled_width, 0),
...
class InfoOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.h_layout.setContentsMargins(
            0, 0, self.h * 0.3, 0
        )  # 这些都要在resize中重写
...
class CoverBrowser(QWidget):
    ...
    def createbuttons(self):
        ...
        self.next_button.setSizePolicy(
            self.next_button.sizePolicy().horizontalPolicy(), QSizePolicy.Expanding
        )
        self.next_button.setCursor(Qt.PointingHandCursor)

        self.next_button.setIcon(
            create_colored_icon_vector(
                str(ICONS_PATH / "chevron-right.svg"), "#CCCCCC", 48, 48
            )
        )
        ...
        self.prev_button.setSizePolicy(
            self.prev_button.sizePolicy().horizontalPolicy(), QSizePolicy.Expanding
        )
        self.prev_button.setCursor(Qt.PointingHandCursor)
        self.prev_button.setIcon(
            create_colored_icon_vector(
                str(ICONS_PATH / "chevron-left.svg"), "#CCCCCC", 48, 48
            )
        )
    ...
    def resizeEvent(self, event):
        ...
        self.info_overlay.setGeometry(
            self.rect()
        )  # 更新覆盖层的大小,使用覆盖层的时候需要重写这个函数去更新大小
...
class CoverTagButton(QPushButton):
    ...
    def __init__(self, tag_id: int, tag_name: str, color: str, detail: str):
        ...
        self.tag_id = tag_id
        self.setStyleSheet(
            f"background-color: {color}; padding: 4px; border-radius: 6px;border: none; outline: none;"
        )
