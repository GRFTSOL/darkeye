...
class SingleActressPage(LazyWidget):
    ...
    def _lazy_load(self):
        ...
        self.lazy_area = LazyScrollArea(
            column_width=220, widget=self.single_actress_info, hint=False
        )
        ...
        for serial_number, title, cover_path, tag_id, work_id, standard in data:
            match tag_id:
                ...
                case None:
                    color = "#00000000"
            card = CoverCard(
                title, cover_path, serial_number, work_id, bool(standard), color=color
            )
